function bgMainStatic(pairNo, player, labName, roundNo, tokenNo, confType, confNo, strangeImgNo, devID)
%% Bargaining Game main experiment
%
% USAGE: bgMainStatic(pairNo, player, labName, roundNo, tokenNo, confType,... 
%                   confNo, strangeImgNo, roundNo, devID)
%
...
if ~ismember(roundNo, 1:99)
    error('Input arg "roundNo" should be one of 1:99!');
end
...
baseDir = "/home/mordor/CommGame/bin/";  % repo
imgDir = "sorted_BOSS_NOUN_rs_100x100/";  % folder in repo containing token images

% get lab mouse device based on hardcoded device name
%labMouseName = "Logitech USB Optical Mouse";
%[mouseIndices, productNames, ~] = GetMouseIndices('slavePointer');
%mouseDev = mouseIndices(ismember(productNames, labMouseName));

%% Video stream settings

% gstreamer spec for reading in webcam stream attached via simple USB
gstSpec = ['v4l2src device=', devID, ' ! jpegdec ! video/x-raw',...
    ',width=1280,height=720,framerate=30/1 ! videoconvert'];
...
vidRecFlags = 20;  % recordingflags arg for OpenVideoCapture, 4 (= only save to disk, do not return image) + 16 (= use parallel thread in background)
...
rand("state", tokenNo+strangeImgNo+roundNo+confNo);
...
% store game params in struct for saving out
gameParams = struct;
gameParams.pairNo = pairNo; gameParams.labName = labName; gameParams.player = player;
gameParams.roundNo = roundNo; gameParams.tokenNo = tokenNo; gameParams.tokenType = confType;
gameParams.confNo = confNo; gameParams.strangeImgNo = strangeImgNo; gameParams.gstSpec = gstSpec;
gameParams.videoSaveFile = videoSaveFile; gameParams.imgDir = imgDir;
gameParams.conf = conf; gameParams.tokenPrices = tokenPrices; gameParams.tokenAmounts = tokenAmounts;
gameParams.mustHaves = mustHaves; gameParams.addButtonState = addButtonState;
gameParams.subtractButtonState = subtractButtonState;
% video-related params
vidParams = struct;
vidParams.moviename = moviename; vidParams.codec = codec;
vidParams.vidDropFrames = vidDropFrames; vidParams.vidRecFlags = vidRecFlags;
vidParams.vidRes = vidRes; vidParams.vidSamplingRate = vidSamplingRate;
vidParams.frameDrawRect = frameDrawRect; vidParams.waitForImage = waitForImage;
% trigger-related params
trigParams = struct;
trigParams.trigL = trigL; trigParams.trigSignal = trigSignal; 
trigParams.trigEndSignal = trigEndSignal; trigParams.trigPeriod = trigPeriod;
% image- and general texture-related params
imgParams = struct;
imgParams.imgLoc = imgLoc; imgParams.imgTargetSize = imgTargetSize; imgParams.priceLoc = priceLoc;
imgParams.priceRectSize = priceRectSize; imgParams.shelvesNoLoc = shelvesNoLoc; 
imgParams.shelvesNoRectSize = shelvesNoRectSize; imgParams.counterLoc = counterLoc;
imgParams.counterRectSize = counterRectSize; imgParams.counterNoLoc = counterNoLoc;
imgParams.counterNoRectSize = counterNoRectSize; imgParams.counterLocOther = counterLocOther;
imgParams.counterNoLocOther = counterNoLocOther; imgParams.bargainLoc = bargainLoc; 
imgParams.bargainRectSize = bargainRectSize;
imgParams.lineLoc = lineLoc; imgParams.lineColor = lineColor; imgParams.lineWidth = lineWidth;
imgParams.backgrColor = backgrColor; imgParams.txtColor = txtColor; imgParams.txtSize = txtSize;
imgParams.totalWealthLoc = totalWealthLoc; imgParams.totalWealthRectSize = totalWealthRectSize;
imgParams.playerOfferLoc = playerOfferLoc; imgParams.playerOfferRectSize = playerOfferRectSize;
imgParams.otherOfferLoc = otherOfferLoc; imgParams.otherOfferRectSize = otherOfferRectSize;
imgParams.mustHavesEndingLoc = mustHavesEndingLoc; imgParams.mustHavesEndingRectSize = mustHavesEndingRectSize;
...
%    SetMouse(xC, yC, onWin, mouseDev);
    ...
    %% Prepare static image as texture
    
    staticImgWin = Screen("OpenOffscreenWindow", onWin, backgrColor);
    staticImgTexture = Screen('MakeTexture', onWin, staticImg);
    Screen('DrawTexture', staticImgWin, staticImgTexture, [], frameDrawRect);  
    ...
    Screen("DrawTexture", onWin, staticImgWin);
    ...
    % preallocate vars for storing all events
    udpPacketsNo = vidLength*vidSamplingRate*10;  % large enough number to cover all incoming udp packets in a game
    bargainStateCode = 1;  % numeric code for "bargainState" message
    endingStateCode = 2;  % numeric code for "endingState" message
    incomingUdpMessages = zeros(udpPacketsNo, 2);  % array to store udp messages (2nd column) and their timestamps (1st coloumn)
    udp_c2s = zeros(udpPacketsNo, counterTypesMax + 1);  % array to store other player's counter2shelves vectors from incoming udp packets (from 2nd column on) and their timestamps (1st coloumn)
    udp_cState = zeros(udpPacketsNo, imgNo + 1);  % array to store other player's counterState vectors from incoming udp packets (from 2nd column on) and their timestamps (1st coloumn)
    udpMessageCounter = 1;
    udp_c2sCounter = 1;
    udp_cStateCounter = 1;
    flipData = struct;
    ...
    flipData.newVidFrameFlag = zeros(length(frameCaptTime), 2);
    ...
    while GetSecs < firstFlip + timeout  && ~mustHavesAllCollected
        ...
        if udpBytesCount ~= -1
            ...
            if numel(tmpCellArray) == 1 && strcmp(tmpCellArray{1}, "bargainState")
                ...
                incomingUdpMessages(udpMessageCounter, :) = [udpReadTime, bargainStateCode];
                udpMessageCounter = udpMessageCounter + 1;
            ...
            elseif numel(tmpCellArray) == 1 && strcmp(tmpCellArray{1}, "endingState")
                % store the message with its timestamp
                incomingUdpMessages(udpMessageCounter, :) = [udpReadTime, endingStateCode];
                udpMessageCounter = udpMessageCounter + 1;
            ...
            % else try to treat it as counter state vectors
                ...
                if ~isequal(incomingVector, previousIncoming)
                    ...
                    % store received c2s and cState vectors with their timestamps
                    udp_c2s(udp_c2sCounter, :) = [udpReadTime, other_c2s'];
                    udp_cState(udp_cStateCounter, :) = [udpReadTime, other_cState'];
                    udp_c2sCounter = udp_c2sCounter + 1;
                    udp_cStateCounter = udp_cStateCounter + 1;
                    % set flag for changes made by other player
                    otherChange = true;
                end
...
%            [xM, yM, buttons] = GetMouse(screenNumber, mouseDev);
            ...
            if any(buttons)
                ...
                if any(mouseInRect)  && ~bargainFlag
                    ...
                    end  % if shelves2counter(mouseInRect)
...
%                    [~, ~, buttons] = GetMouse(screenNumber, mouseDev);
            ...
            if bargainFlag
                udpMessage = "bargainState";
                % only send it once, will repeatedly do so due to the while loop anyways
                send(udpSocket, udpMessage);
            end  % if
            ...
            if changeFlag  || otherChange
                ...
                %    % store explicitly that a new video frame is in texture for current screen flip
                %    flipData.newVidFrameFlag(flipCounter, 1:2) = [1, vidFrameCount];
                ...
                %    % store explicitly that there is no new video frame in the texture for current screen flip
                %    flipData.newVidFrameFlag(flipCounter, 1:2) = [0, vidFrameCount];                
                %end  % if tex                
    ...
    save("-v7", videoSaveFile, "sharedStartTime", "vidCaptureStartTime",...
    "frameCaptTime", "vidFrameCount", "flipTimeStamps", "stopCaptureTime",...
    "closeCaptureTime", "flipData", "gameParams", "vidParams", "trigParams", ...
    "imgParams");   
